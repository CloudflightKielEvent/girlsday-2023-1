...
export class LevelSystem {
    ...
    static checkProgressCondition() {
        ...
        if (GlobalGameState.current.score > 7 && SceneManager.currentScene.name === GlobalConfig.INITIAL_SCENE_NAME) {
        ...
        if (GlobalGameState.current.score > 14 && SceneManager.currentScene.name === GlobalConfig.SECOND_SCENE_NAME) {
            LevelSystem.switchLevel(GlobalConfig.THIRD_SCENE_NAME);

            return true;
        }

        // Switch to fourth level
        if (GlobalGameState.current.score > 21 && SceneManager.currentScene.name === GlobalConfig.THIRD_SCENE_NAME) {
            LevelSystem.switchLevel(GlobalConfig.FOURTH_SCENE_NAME);
